# Calendar Linking

## What this is

Link one or more calendars to a workspace.

## Current setup

- Home can sync from more than one Google calendar ID.
- The app merges those events into the same calendar and schedule view.
- Untimed events still become to-dos for that day.

## Keep it simple

1. Add the Google Client ID.
2. Add the calendar IDs you want, separated by commas or new lines.
3. Turn on sync.
4. Click sync now.

## Private data rule

- Keep tokens and personal calendar data out of Git.
